...
| gui   | laptop         | Firefox Developer Edition, Thunderbird Beta, WezTerm (nightly), VS Code Insiders, Obsidian, Evolution (+ EWS), Google Chrome, Slack, Zoom, ParaView, VLC, Zotero, Clockify, LibreOffice (+ en-US help), Spotify, OneDrive client, libfuse2t64 (AppImage support) |
...
On every machine, bootstrap validates `lib/90-dotfiles-nopasswd` and installs
it as `/etc/sudoers.d/90-dotfiles-nopasswd`, owned by root with mode `0440`.
It grants `NOPASSWD: ALL` to Ubuntu's `sudo` group. The first bootstrap may
need the account's existing sudo password before the policy is installed;
later sudo commands and bootstrap runs do not prompt.
